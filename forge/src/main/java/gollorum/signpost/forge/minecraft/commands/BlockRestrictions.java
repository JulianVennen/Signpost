...
public class BlockRestrictions {
	...
	private static ArgumentBuilder<CommandSourceStack, ?> signposts() {
		return LiteralArgumentBuilder.<CommandSourceStack>literal("signposts")
			.then(getter(gollorum.signpost.forge.BlockRestrictions.Type.Signpost))
			.then(setter(gollorum.signpost.forge.BlockRestrictions.Type.Signpost));
	}
	...
	private static ArgumentBuilder<CommandSourceStack, ?> waystone() {
		return LiteralArgumentBuilder.<CommandSourceStack>literal("waystone")
			.then(getter(gollorum.signpost.forge.BlockRestrictions.Type.Waystone))
			.then(setter(gollorum.signpost.forge.BlockRestrictions.Type.Waystone));
	}

	private static ArgumentBuilder<CommandSourceStack, ?> getter(gollorum.signpost.forge.BlockRestrictions.Type type) {
	...
	private static int get(
		gollorum.signpost.forge.BlockRestrictions.Type type, CommandSourceStack commandSource, Player targetedPlayer
	) {
		int left = gollorum.signpost.forge.BlockRestrictions.getInstance().getRemaining(type, PlayerHandle.from(targetedPlayer));
	...
	private static ArgumentBuilder<CommandSourceStack, ?> setter(gollorum.signpost.forge.BlockRestrictions.Type type) {
	...
	private static int set(gollorum.signpost.forge.BlockRestrictions.Type type, CommandSourceStack commandSource, ServerPlayer targetedPlayer, int count) {
		PlayerHandle tHandle = PlayerHandle.from(targetedPlayer);
		gollorum.signpost.forge.BlockRestrictions.getInstance().setRemaining(type, tHandle, c -> count);
