package gollorum.signpost.forge.compat.waystones;
...
public final class WaystonesAdapter implements ExternalWaystoneLibrary.Adapter {
    ...
    public static void register() {
        ...
        PacketHandler.onInitialize().addListener(unit -> {
            PacketHandler.register(new RequestEvent());
            PacketHandler.register(new ReplyEvent());
        });
    }
    ...
    private Optional<Waystone> getData(Handle handle) {
        return WaystoneManager.get(ServerHolder.getServer()).getWaystoneById(handle.id).map(Waystone::new);
    }
    ...
    public static final class RequestEvent extends ReflectionEvent<RequestEvent> {
        ...
        @Override
        public void handle(RequestEvent message, NetworkManager.PacketContext context) {
            PacketHandler.send(
                (ServerPlayer) context.getPlayer(),
                new ReplyEvent.Packet(PlayerWaystoneManager.getWaystones(context.getPlayer())
                    .stream()
                    .map(Waystone::new)
