...
public class WaystoneArgument implements ArgumentType<String> {
	private static final Registrar<ArgumentTypeInfo<?, ?>> ARGUMENT_TYPES = Signpost.REGISTRAR_MANAGER.get().get(Registries.COMMAND_ARGUMENT_TYPE);

	private static final Info INFO = new Info();

	static {
		ArgumentTypeInfos.BY_CLASS.put(WaystoneArgument.class, INFO);
		ARGUMENT_TYPES.register(new ResourceLocation(MOD_ID, "waystone"), () -> INFO);
	}
	...
	public <S> CompletableFuture<Suggestions> listSuggestions(
		...
		WaystoneLibrary.getInstance().requestAllWaystoneNames(
			...
			Optional.empty(),
			ServerHolder.getServerType().isClient
		);
		return ret;
	}
