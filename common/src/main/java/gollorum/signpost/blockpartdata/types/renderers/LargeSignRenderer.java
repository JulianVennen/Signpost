...
public class LargeSignRenderer extends SignRenderer<LargeSignBlockPart> {
	...
	public void renderText(LargeSignBlockPart sign, PoseStack matrix, Font fontRenderer, MultiBufferSource buffer, int combinedLights) {
		Signpost.getRenderingUtil().wrapInMatrixEntry(matrix, () -> {
			...
			Signpost.getRenderingUtil().wrapInMatrixEntry(matrix, () -> render(sign, fontRenderer, sign.getText()[3].get(), matrix, buffer, combinedLights, false));
			matrix.translate(0, -7 / 3f * VoxelSize, 0);

			Signpost.getRenderingUtil().wrapInMatrixEntry(matrix, () -> render(sign, fontRenderer, sign.getText()[2].get(), matrix, buffer, combinedLights, false));
			matrix.translate(0, -7 / 3f * VoxelSize, 0);

			Signpost.getRenderingUtil().wrapInMatrixEntry(matrix, () -> render(sign, fontRenderer, sign.getText()[1].get(), matrix, buffer, combinedLights, false));
			matrix.translate(0, -7 / 3f * VoxelSize, 0);

			Signpost.getRenderingUtil().wrapInMatrixEntry(matrix, () -> render(sign, fontRenderer, sign.getText()[0].get(), matrix, buffer, combinedLights, false));
		});
	}
